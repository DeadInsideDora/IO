...
#define SEC(x) (((x) % SEC_PER_HEAD) + 1)
#define HEAD(x) ((((x) - SEC(x) + 1) / SEC_PER_HEAD) % HEAD_PER_CYL)
#define CYL(x) (((x) - (SEC(x) - 1) - HEAD(x) * SEC_PER_HEAD) / (SEC_PER_HEAD * HEAD_PER_CYL))
        ...
        {
            {
                boot_type: 0x00,
                start_sec: SEC(0x01),
                start_head: HEAD(0x01),
                start_cyl: CYL(0x01),
                part_type: 0x83,
                end_head: HEAD(0xa000),
                end_sec: SEC(0xa000),
                end_cyl: CYL(0xa000),
                abs_start_sec: 0x1,
                sec_in_part: 0xa000
            ...
            {
                boot_type: 0x00,
                start_head: HEAD(0xa001),
                start_sec: SEC(0xa001),
                start_cyl: CYL(0xa001),
                part_type: 0x05, // extended partition type
                end_sec: SEC(MEMSIZE),
                end_head: HEAD(MEMSIZE),
                end_cyl: CYL(MEMSIZE),
...
static unsigned int def_log_part_br_abs_start_sector[] = {0xa001, 0x12801};
    ...
    {
        {
            ...
            {
                boot_type: 0x00,
                start_head: HEAD(0x8801),
                start_sec: SEC(0x8801),
                start_cyl: CYL(0x8801),
                part_type: 0x05,
                end_head: HEAD(0xf000),
                end_sec: SEC(0xf000),
                end_cyl: CYL(0xf000),
...
MODULE_DESCRIPTION("Our block driver");
